 ...
 * (<code>{@link org.quartz.simpl.SimpleTimeBroker}</code>- which simply uses
 * <code>System.getCurrentTimeMillis()</code> )is sufficient. However situations
 * may exist where this default scheme is lacking in its robustness - especially
 * when Quartz is used in a clustered configuration. For example, if one or more
 * of the machines in the cluster has a system time that varies by more than a
 * few seconds from the clocks on the other systems in the cluster, scheduling
...
public interface TimeBroker
{
  ...
  /**
   * <p>
   * Called by the QuartzScheduler before the <code>TimeBroker</code> is used,
  ...
  /**
   * <p>
   * Called by the QuartzScheduler to inform the <code>TimeBroker</code> that it
