...
public interface QuartzSchedulerMBean
{
  ...
  /**
   * Schedules a job using the given Cron/Simple triggerInfo. The triggerInfo
   * and jobDetailInfo must contain well-known attribute values. TriggerInfo
   * attributes: name, group, description, calendarName, priority,
   * CronExpression | (startTime, endTime, repeatCount, repeatInterval)
   * JobDetailInfo attributes: name, group, description, jobClass, jobDataMap,
  ...
  /**
   * Schedules an arbitrary job described by abstractJobInfo using a trigger
   * specified by abstractTriggerInfo. AbtractTriggerInfo and AbstractJobInfo
   * must contain the following String attributes. AbstractTriggerInfo:
   * triggerClass, the fully-qualified class name of a concrete Trigger type
   * AbstractJobInfo: jobDetailClass, the fully-qualified class name of a
   * concrete JobDetail type If the Trigger and JobDetail can be successfully
   * instantiated, the remaining attributes will be reflectively applied to
   * those instances. The remaining attributes are limited to the types:
   * Integer, Double, Float, String, Boolean, Date, Character, Map<String,
   * Object>. Maps are further limited to containing values from the same set of
   * types, less Map itself.
   *
   * @throws Exception
   */
  ...
  /**
   * Schedules the specified job using a trigger described by
   * abstractTriggerInfo, which must contain the fully-qualified trigger class
   * name under the key "triggerClass." That trigger type must contain a no-arg
   * constructor and have public access. Other attributes are applied
   * reflectively and are limited to the types: Integer, Double, Float, String,
   * Boolean, Date, Character, Map<String, Object>. Maps are limited to
   * containing values from the same set of types, less Map itself.
   *
   * @param jobName
   * @param jobGroup
  ...
  /**
   * Adds a durable job described by abstractJobInfo, which must contain the
   * fully-qualified JobDetail class name under the key "jobDetailClass." That
   * JobDetail type must contain a no-arg constructor and have public access.
   * Other attributes are applied reflectively and are limited to the types:
   * Integer, Double, Float, String, Boolean, Date, Character, Map<String,
   * Object>. Maps are limited to containing values from the same set of types,
   * less Map itself.
   *
   * @param abstractJobInfo
   *        map of attributes defining job
   * @param replace
   *        whether or not to replace a pre-existing job with the same key
   * @throws Exception
   */
  ...
  /**
   * @return List of CompositeData:[CronTrigger|SimpleTrigger] for the specified
   *         job.
   * @see TriggerSupport
   */
