...
public class JobExecutionContextSupport
{
  ...
  private static final String [] ITEM_NAMES = new String [] { "schedulerName",
                                                              ...
                                                              "jobRunTime",
                                                              "fireInstanceId" };
  private static final String [] ITEM_DESCRIPTIONS = new String [] { "schedulerName",
                                                                     "triggerName",
                                                                     "triggerGroup",
                                                                     "jobName",
                                                                     "jobGroup",
   ...
   */
  public static CompositeData toCompositeData (final JobExecutionContext jec) throws SchedulerException
  {
    try
    {
      return new CompositeDataSupport (COMPOSITE_TYPE,
                                       ITEM_NAMES,
                                       new Object [] { jec.getScheduler ().getSchedulerName (),
                                                       ...
                                                       JobDataMapSupport.toTabularData (jec.getMergedJobDataMap ()),
                                                       jec.getTrigger ().getCalendarName (),
                                                       jec.isRecovering (),
                                                       jec.getRefireCount (),
                                                       jec.getFireTime (),
                                                       jec.getScheduledFireTime (),
                                                       jec.getPreviousFireTime (),
                                                       jec.getNextFireTime (),
                                                       jec.getJobRunTime (),
  ...
  {
    final List <CompositeData> list = new ArrayList <> ();
    for (final JobExecutionContext executingJob : executingJobs)
    {
      list.add (toCompositeData (executingJob));
    }
    final TabularData td = new TabularDataSupport (TABULAR_TYPE);
