 ...
 * org.quartz.impl.jdbcjobstore.JobStoreTX}</code> instead.
 * </p>
 *
...
public class JobStoreCMT extends JobStoreSupport
{
   ...
   * </p>
   */
  public void setNonManagedTXDataSource (final String nonManagedTxDsName)
  {
   ...
   * DataSource. This can be helpfull in a few situations, such as if you have a
   * driver that complains if it is called when it is already off.
   *
   * @param b
   */
  public void setDontSetNonManagedTXConnectionAutoCommitFalse (final boolean b)
  {
   ...
   * @param b
   */
  public void setTxIsolationLevelReadCommitted (final boolean b)
  {
  ...
  protected Connection getNonManagedTXConnection () throws JobPersistenceException
  {
    ...
    if (conn == null)
    {
      throw new JobPersistenceException ("Could not get connection from DataSource '" +
                                         getNonManagedTXDataSource () +
                                         "'");
    }
   ...
   * transaction, it does not attempt to commit or rollback the enclosing
   * transaction.
   *
   * @param lockName
   *        The name of the lock to acquire, for example "TRIGGER_ACCESS". If
   *        null, then no lock is acquired, but the txCallback is still executed
   *        in a transaction.
   ...
   */
  @Override
  protected Object executeInLock (final String lockName,
                                  final TransactionCallback txCallback) throws JobPersistenceException
  {
