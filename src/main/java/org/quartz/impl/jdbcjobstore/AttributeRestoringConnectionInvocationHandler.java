...
/**
 * <p>
 * Protects a <code>{@link java.sql.Connection}</code>'s attributes from being
 * permanently modfied.
 * </p>
 * <p>
 * Wraps a provided <code>{@link java.sql.Connection}</code> such that its auto
 * commit and transaction isolation attributes can be overwritten, but will
 * automatically restored to their original values when the connection is
 * actually closed (and potentially returned to a pool for reuse).
 * </p>
 *
...
public class AttributeRestoringConnectionInvocationHandler implements InvocationHandler
{
  ...
  public AttributeRestoringConnectionInvocationHandler (final Connection conn)
  {
    this.conn = conn;
  }
  ...
  /**
   * Sets this connection's auto-commit mode to the given state, saving the
   * original mode. The connection's original auto commit mode is restored when
   * the connection is closed.
   */
  public void setAutoCommit (final boolean autoCommit) throws SQLException
  {
    final boolean currentAutoCommitValue = conn.getAutoCommit ();
  ...
  /**
   * Attempts to change the transaction isolation level to the given level,
   * saving the original level. The connection's original transaction isolation
   * level is restored when the connection is closed.
   */
  public void setTransactionIsolation (final int level) throws SQLException
  {
    final int currentLevel = conn.getTransactionIsolation ();
   ...
   * This is provided in case a user ever needs to punch through the wrapper to
   * access vendor specific methods outside of the standard
   * <code>java.sql.Connection</code> interface.
   *
   * @return The underlying connection to which all operations ultimately defer.
   */
   ...
   * attributes of the wrapped connection to their original values (if they were
   * overwritten).
   */
   ...
   * Attempts to restore the auto commit and transaction isolation connection
   * attributes of the wrapped connection to their original values (if they were
   * overwritten), before finally actually closing the wrapped connection.
   */
