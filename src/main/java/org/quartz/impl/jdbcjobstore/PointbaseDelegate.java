...
public class PointbaseDelegate extends StdJDBCDelegate
{
                            ...
                            final JobDetail jobDetail) throws SQLException, IOException
  {
    ...
    final ByteArrayOutputStream baos = serializeJobData (trigger.getJobDataMap ());
    final int len = baos.toByteArray ().length;
    final ByteArrayInputStream bais = new ByteArrayInputStream (baos.toByteArray ());
                                                                                ...
                                                                                SQLException
  {
    ...
    final byte binaryData[] = rs.getBytes (colName);

    final InputStream binaryInput = new ByteArrayInputStream (binaryData);
                                                                                 ...
                                                                                 SQLException
  {
    ...
    if (canUseProperties ())
    {
      final byte data[] = rs.getBytes (colName);
      if (data == null)
      {
        return null;
      }
      final InputStream binaryInput = new ByteArrayInputStream (data);
      return binaryInput;
    }
