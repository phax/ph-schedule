...
/**
 * Provide thread/resource locking in order to protect resources from being
 * altered by multiple threads at the same time using a db row update.
 * <p>
...
public class UpdateLockRowSemaphore extends DBSemaphore
{
  ...
  public static final String UPDATE_FOR_LOCK = "UPDATE " +
                                               TABLE_PREFIX_SUBST +
                                               TABLE_LOCKS +
                                               " SET " +
                                               COL_LOCK_NAME +
   ...
   */
  @Override
  protected void executeSQL (final Connection conn,
                             final String lockName,
                             final String expandedSQL,
                             final String expandedInsertSQL) throws LockException
  {
    ...
    for (int i = 0; i < RETRY_COUNT; i++)
    {
      ...
      catch (final SQLException e)
      {
        ...
        {
          getLog ().debug ("Lock '{}' was not obtained by: {} - will try again.",
                           lockName,
  ...
  public void setUpdateLockRowSQL (final String updateLockRowSQL)
  {
    setSQL (updateLockRowSQL);
  }

  private boolean lockViaUpdate (final Connection conn, final String lockName, final String sql) throws SQLException
  {
  ...
  private void lockViaInsert (final Connection conn, final String lockName, final String sql) throws SQLException
  {
    getLog ().debug ("Inserting new lock row for lock: '" +
                     lockName +
                     "' being obtained by thread: " +
                     Thread.currentThread ().getName ());
    final PreparedStatement ps = conn.prepareStatement (sql);
    try
    {
      ...
      {
        throw new SQLException (Util.rtp ("No row exists, and one could not be inserted in table " +
                                          TABLE_PREFIX_SUBST +
                                          TABLE_LOCKS +
                                          " for lock named: " +
                                          lockName,
