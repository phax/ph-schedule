...
/**
 * Internal database based lock handler for providing thread/resource locking in
 * order to protect resources from being altered by multiple threads at the same
 * time.
 *
 * @author jhouse
 */
public class StdRowLockSemaphore extends DBSemaphore
{
  ...
  public StdRowLockSemaphore (final String tablePrefix, final String schedName, final String selectWithLockSQL)
  {
                             ...
                             final String expandedInsertSQL) throws LockException
  {
    ...
    do
    {
      ...
      try
      {
        ...
        if (!rs.next ())
        {
          ...
          if (res != 1)
          {
            if (count < 3)
            {
              // pause a bit to give another thread some time to commit the
              // insert of the new lock row
              try
              {
                Thread.sleep (1000L);
              }
      ...
      catch (final SQLException sqle)
      {
        ...
        if (count < 3)
        {
          // pause a bit to give another thread some time to commit the insert
          // of the new lock row
          try
          {
            Thread.sleep (1000L);
  ...
  public void setSelectWithLockSQL (final String selectWithLockSQL)
  {
