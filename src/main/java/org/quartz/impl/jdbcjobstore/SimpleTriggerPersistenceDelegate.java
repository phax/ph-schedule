...
public class SimpleTriggerPersistenceDelegate implements TriggerPersistenceDelegate, StdJDBCConstants
{
                                                              ...
                                                              final TriggerKey triggerKey) throws SQLException
  {
    ...
    try
    {
      ...
      if (rs.next ())
      {
        final int repeatCount = rs.getInt (COL_REPEAT_COUNT);
        final long repeatInterval = rs.getLong (COL_REPEAT_INTERVAL);
        final int timesTriggered = rs.getInt (COL_TIMES_TRIGGERED);
