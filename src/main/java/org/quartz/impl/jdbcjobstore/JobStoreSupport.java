...
public abstract class JobStoreSupport implements JobStore, Constants
{
   ...
   * </p>
   */
  public void setDataSource (final String dsName)
  {
    this.dsName = dsName;
  }
  ...
  /**
   * Set the instance name of the Scheduler (must be unique within this server
   * instance).
   */
  ...
  /**
   * Get the instance name of the Scheduler (must be unique within this server
   * instance).
   */
  ...
  /**
   * <p>
   * Get the frequency (in milliseconds) at which this instance "checks-in" with
   * the other instances of the cluster. -- Affects the rate of detecting failed
   * instances.
   * </p>
   */
   ...
   */
  /* called reflectively */
  public void setClusterCheckinInterval (final long l)
  {
   ...
   */
  /* called reflectively */
  public void setMaxMisfiresToHandleAtATime (final int maxToRecoverAtATime)
  {
   ...
   */
  public void setDbRetryInterval (final long dbRetryInterval)
  {
   ...
   * </p>
   */
  public void setUseDBLocks (final boolean useDBLocks)
  {
   ...
   * <p>
   * Defaults to <code>true</code>, which is safest. Some databases (such as MS
   * SQLServer) seem to require this to avoid deadlocks under high load, while
   * others seem to do fine without. Settings this to false means isolation
   * guarantees between job scheduling and trigger acquisition are entirely
   * enforced by the database. Depending on the database and it's configuration
   * this may cause unusual scheduling behaviors.
   * <p>
   * Setting this property to <code>false</code> will provide a significant
   * performance increase during the addition of new jobs and triggers.
   * </p>
   *
   * @param lockOnInsert
   ...
   */
  /* called reflectively */
  public void setLockOnInsert (final boolean lockOnInsert)
  {
   ...
   * DataSource. This can be helpful in a few situations, such as if you have a
   * driver that complains if it is called when it is already off.
   *
   * @param b
   *        whether or not autocommit should be set to false on db connections
   */
  /* called reflectively */
  public void setDontSetAutoCommitFalse (final boolean b)
  {
   ...
   */
  /* called reflectively */
  public void setTxIsolationLevelSerializable (final boolean b)
  {
    setTxIsolationLevelSequential = b;
  }

  /**
   * Whether or not the query and update to acquire a Trigger for firing should
   * be performed after obtaining an explicit DB lock (to avoid possible race
   * conditions on the trigger's db row). This is the behavior prior to Quartz
   * 1.6.3, but is considered unnecessary for most databases (due to the nature
   * of the SQL update that is performed), and therefore a superfluous
   * performance hit.
   */
  ...
  /**
   * Whether or not the query and update to acquire a Trigger for firing should
   * be performed after obtaining an explicit DB lock. This is the behavior
   * prior to Quartz 1.6.3, but is considered unnecessary for most databases,
   * and therefore a superfluous performance hit. However, if batch acquisition
   * is used, it is important for this behavior to be used for all dbs.
   */
  /* called reflectively */
  public void setAcquireTriggersWithinLock (final boolean acquireTriggersWithinLock)
  {
   ...
   */
  /* called reflectively */
  public void setDriverDelegateClass (final String delegateClassName) throws InvalidConfigurationException
  {
    synchronized (this)
    {
   ...
   */
  /* called reflectively */
  public void setDriverDelegateInitString (final String delegateInitString) throws InvalidConfigurationException
  {
   ...
   */
  public void setSelectWithLockSQL (final String string)
  {
  ...
  /**
   * Get whether the threads spawned by this JobStore should be marked as
   * daemon. Possible threads include the <code>MisfireHandler</code> and the
  ...
  /**
   * Set whether the threads spawned by this JobStore should be marked as
   * daemon. Possible threads include the <code>MisfireHandler</code> and the
   * <code>ClusterManager</code>.
   *
   * @see Thread#setDaemon(boolean)
  ...
  /**
   * Get whether to check to see if there are Triggers that have misfired before
   * actually acquiring the lock to recover them. This should be set to false if
   * the majority of the time, there are are misfired Triggers.
   */
  ...
  /**
   * Set whether to check to see if there are Triggers that have misfired before
   * actually acquiring the lock to recover them. This should be set to false if
   * the majority of the time, there are are misfired Triggers.
   */
  /* called reflectively */
  public void setDoubleCheckLockMisfireHandler (final boolean doubleCheckLockMisfireHandler)
  {
   ...
   * order to give it a chance to initialize.
   * </p>
   */
  public void initialize (final ClassLoadHelper loadHelper,
                          final SchedulerSignaler signaler) throws SchedulerConfigException
  {
    ...
    {
      log.info ("JDBCJobStore threads will inherit ContextClassLoader of thread: " +
  ...
  /**
   * Wrap the given <code>Connection</code> in a Proxy such that attributes that
   * might be set will be restored before the connection is closed (and
   * potentially restored to a pool).
   */
  protected Connection getAttributeRestoringConnection (final Connection conn)
  {
  ...
  protected void releaseLock (final String lockName, final boolean doIt)
  {
    if (doIt)
    {
      try
  ...
  protected void recoverJobs () throws JobPersistenceException
  {
    ...
    {
      @Override
      public void executeVoid (final Connection conn) throws JobPersistenceException
      {
        recoverJobs (conn);
      }
    }, null);
  ...
  protected void recoverJobs (final Connection conn) throws JobPersistenceException
  {
    try
    {
      ...
      final List <OperableTrigger> recoveringJobTriggers = getDelegate ().selectTriggersForRecoveringJobs (conn);
      getLog ().info ("Recovering " +
                      recoveringJobTriggers.size () +
                      " jobs that were in-progress at the time of the last shut-down.");

      for (final OperableTrigger recoveringJobTrigger : recoveringJobTriggers)
      {
      ...
      final List <TriggerKey> cts = getDelegate ().selectTriggersInState (conn, STATE_COMPLETE);
      for (final TriggerKey ct : cts)
      {
      ...
      final int n = getDelegate ().deleteFiredTriggers (conn);
  ...
  /**
   * Helper class for returning the composite result of trying to recover
   * misfired jobs.
   */
  protected static class RecoverMisfiredJobsResult
  {
    ...
    public RecoverMisfiredJobsResult (final boolean hasMoreMisfiredTriggers,
                                      final int processedMisfiredTriggerCount,
                                      final long earliestNewTime)
    {
  ...
  protected RecoverMisfiredJobsResult recoverMisfiredJobs (final Connection conn, final boolean recovering)
                                                                                                            throws JobPersistenceException,
                                                                                                            SQLException
  {
    ...
    // triggers right away.
    final int maxMisfiresToHandleAtATime = (recovering) ? -1 : getMaxMisfiresToHandleAtATime ();

    final List <TriggerKey> misfiredTriggers = new LinkedList<> ();
    long earliestNewTime = Long.MAX_VALUE;
    // We must still look for the MISFIRED state in case triggers were left
    // in this state when upgrading to this version that does not support it.
    final boolean hasMoreMisfiredTriggers = getDelegate ().hasMisfiredTriggersInState (conn,
                                                                                       STATE_WAITING,
                                                                                       getMisfireTime (),
    ...
    {
      getLog ().info ("Handling the first " +
                      misfiredTriggers.size () +
    ...
    for (final TriggerKey triggerKey : misfiredTriggers)
    {

      final OperableTrigger trig = retrieveTrigger (conn, triggerKey);
  ...
  protected boolean updateMisfiredTrigger (final Connection conn,
                                           final TriggerKey triggerKey,
                                           final String newStateIfNotComplete,
                                           final boolean forceState) throws JobPersistenceException
  {
    try
    {
    ...
    {
      throw new JobPersistenceException ("Couldn't update misfired trigger '" +
                                         triggerKey +
                                         "': " +
                                         e.getMessage (),
                                         e);
  ...
  private void doUpdateOfMisfiredTrigger (final Connection conn,
                                          final OperableTrigger trig,
   ...
   * </p>
   */
  protected void storeJob (final Connection conn,
                           final JobDetail newJob,
                           final boolean replaceExisting) throws JobPersistenceException
  {

    final boolean existingJob = jobExists (conn, newJob.getKey ());
    try
    {
      if (existingJob)
      {
   ...
   * </p>
   */
  protected boolean jobExists (final Connection conn, final JobKey jobKey) throws JobPersistenceException
  {
    try
    {
                               ...
                               final boolean recovering) throws JobPersistenceException
  {
    ...
    final boolean existingTrigger = triggerExists (conn, newTrigger.getKey ());
   ...
   * </p>
   */
  protected boolean triggerExists (final Connection conn, final TriggerKey key) throws JobPersistenceException
  {
    try
    {
  ...
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> removeJob (conn, jobKey) ? Boolean.TRUE : Boolean.FALSE);
  }

  protected boolean removeJob (final Connection conn, final JobKey jobKey) throws JobPersistenceException
  {

    try
    {
      final List <TriggerKey> jobTriggers = getDelegate ().selectTriggerKeysForJob (conn, jobKey);
      for (final TriggerKey jobTrigger : jobTriggers)
      {
                                    ...
                                    final boolean replace) throws JobPersistenceException
  {
    ...
    executeInLock ((isLockOnInsert () || replace) ? LOCK_TRIGGER_ACCESS : null, new VoidTransactionCallback ()
    {
      ...
      public void executeVoid (final Connection conn) throws JobPersistenceException
      {
        ...
        // FUTURE_TODO: make this more efficient with a true bulk operation...
        for (final JobDetail job : triggersAndJobs.keySet ())
        {
          storeJob (conn, job, replace);
          for (final Trigger trigger : triggersAndJobs.get (job))
          {
   ...
   */
  private boolean deleteJobAndChildren (final Connection conn, final JobKey key) throws NoSuchDelegateException,
                                                                                 SQLException
  {
   ...
   */
  private boolean deleteTriggerAndChildren (final Connection conn, final TriggerKey key) throws SQLException,
  ...
  public JobDetail retrieveJob (final JobKey jobKey) throws JobPersistenceException
  {
    return executeWithoutLock ( // no locks necessary for read...
                                conn -> retrieveJob (conn, jobKey));
  }

  protected JobDetail retrieveJob (final Connection conn, final JobKey key) throws JobPersistenceException
  {
    try
    {
  ...
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> removeTrigger (conn, triggerKey) ? Boolean.TRUE : Boolean.FALSE);
  }

  protected boolean removeTrigger (final Connection conn, final TriggerKey key) throws JobPersistenceException
  {
    boolean removedTrigger;
    try
    {
      // this must be called before we delete the trigger, obviously
      final JobDetail job = getDelegate ().selectJobForTrigger (conn, getClassLoadHelper (), key, false);

      removedTrigger = deleteTriggerAndChildren (conn, key);

      if (null != job && !job.isDurable ())
      {
        final int numTriggers = getDelegate ().selectNumTriggersForJob (conn, job.getKey ());
        if (numTriggers == 0)
        {
                                 ...
                                 final OperableTrigger newTrigger) throws JobPersistenceException
  {
    return executeInLock (LOCK_TRIGGER_ACCESS,
                          conn -> replaceTrigger (conn, triggerKey, newTrigger) ? Boolean.TRUE : Boolean.FALSE);
  }

  protected boolean replaceTrigger (final Connection conn,
                                    ...
                                    final OperableTrigger newTrigger) throws JobPersistenceException
  {
    ...
    {
      // this must be called before we delete the trigger, obviously
      final JobDetail job = getDelegate ().selectJobForTrigger (conn, getClassLoadHelper (), key);
      ...
      final boolean removedTrigger = deleteTriggerAndChildren (conn, key);
  ...
  {
    return executeWithoutLock ( // no locks necessary for
                                // read...
                                conn -> retrieveTrigger (conn, triggerKey));
  }

  protected OperableTrigger retrieveTrigger (final Connection conn, final TriggerKey key) throws JobPersistenceException
  {
    try
    {
  ...
  {
    return executeWithoutLock ( // no locks necessary for read...
                                conn -> getTriggerState (conn, triggerKey));
  }

  public TriggerState getTriggerState (final Connection conn, final TriggerKey key) throws JobPersistenceException
  {
    try
    {
      final String ts = getDelegate ().selectTriggerState (conn, key);
  ...
  public void storeCalendar (final String calName,
                             ...
                             final boolean replaceExisting,
                             final boolean updateTriggers) throws JobPersistenceException
  {
  ...
  protected void storeCalendar (final Connection conn,
                                ...
                                final boolean updateTriggers) throws JobPersistenceException
  {
    try
    {
      final boolean existingCal = calendarExists (conn, calName);
      ...
      if (existingCal)
      {
        ...
        if (updateTriggers)
        {
          final List <OperableTrigger> trigs = getDelegate ().selectTriggersForCalendar (conn, calName);

          for (final OperableTrigger trigger : trigs)
          {
    ...
    {
      throw new JobPersistenceException ("Couldn't store calendar because the BLOB couldn't be serialized: " +
                                         e.getMessage (),
                                         e);
    }
    catch (final ClassNotFoundException e)
  ...
  protected boolean calendarExists (final Connection conn, final String calName) throws JobPersistenceException
  {
    try
    {
    ...
    {
      throw new JobPersistenceException ("Couldn't determine calendar existence (" +
                                         calName +
                                         "): " +
                                         e.getMessage (),
                                         e);
  ...
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> removeCalendar (conn, calName) ? Boolean.TRUE : Boolean.FALSE);
  }

  protected boolean removeCalendar (final Connection conn, final String calName) throws JobPersistenceException
  {
    try
    {
  ...
  protected Calendar retrieveCalendar (final Connection conn, final String calName) throws JobPersistenceException
  {
    ...
    {
      throw new JobPersistenceException ("Couldn't retrieve calendar because a required class was not found: " +
                                         e.getMessage (),
                                         e);
    }
    catch (final IOException e)
    {
      throw new JobPersistenceException ("Couldn't retrieve calendar because the BLOB couldn't be deserialized: " +
                                         e.getMessage (),
                                         e);
    }
    catch (final SQLException e)
  ...
  /**
   * <p>
   * Get the number of <code>{@link org.quartz.Job}</code> s that are stored in
  ...
  {
    return executeWithoutLock ( // no locks necessary for read...
                                conn -> getNumberOfJobs (conn));
  }

  protected int getNumberOfJobs (final Connection conn) throws JobPersistenceException
  {
    try
    {
  ...
  protected int getNumberOfTriggers (final Connection conn) throws JobPersistenceException
  {
    try
    {
  ...
  {
    return executeWithoutLock ( // no locks necessary for read...
                                conn -> getNumberOfCalendars (conn));
  }

  protected int getNumberOfCalendars (final Connection conn) throws JobPersistenceException
  {
    try
    {
  ...
  {
    return executeWithoutLock ( // no locks necessary for read...
                                conn -> getJobNames (conn, matcher));
  }

  protected Set <JobKey> getJobNames (final Connection conn,
                                      final GroupMatcher <JobKey> matcher) throws JobPersistenceException
  {
    Set <JobKey> jobNames;
  ...
  protected boolean checkExists (final Connection conn, final JobKey jobKey) throws JobPersistenceException
  {
    try
    {
      return getDelegate ().jobExists (conn, jobKey);
  ...
  protected boolean checkExists (final Connection conn, final TriggerKey triggerKey) throws JobPersistenceException
  {
    try
    {
  ...
  protected void clearAllSchedulingData (final Connection conn) throws JobPersistenceException
  {
    try
    {
   ...
   * </p>
   * <p>
   * If there are no triggers in the given group name, the result should be a an
  ...
  public Set <TriggerKey> getTriggerKeys (final GroupMatcher <TriggerKey> matcher) throws JobPersistenceException
  {
    return executeWithoutLock ( // no locks necessary for
                                // read...
                                conn -> getTriggerNames (conn, matcher));
  }

  protected Set <TriggerKey> getTriggerNames (final Connection conn,
                                              final GroupMatcher <TriggerKey> matcher) throws JobPersistenceException
  {
  ...
  public List <String> getJobGroupNames () throws JobPersistenceException
  {
    return executeWithoutLock ( // no locks necessary for
                                // read...
                                conn -> getJobGroupNames (conn));
  }

  protected List <String> getJobGroupNames (final Connection conn) throws JobPersistenceException
  {
  ...
  public List <String> getTriggerGroupNames () throws JobPersistenceException
  {
    return executeWithoutLock ( // no locks necessary for
                                // read...
                                conn -> getTriggerGroupNames (conn));
  }

  protected List <String> getTriggerGroupNames (final Connection conn) throws JobPersistenceException
  {
  ...
  public List <String> getCalendarNames () throws JobPersistenceException
  {
    return executeWithoutLock ( // no locks necessary for
                                // read...
                                conn -> getCalendarNames (conn));
  }

  protected List <String> getCalendarNames (final Connection conn) throws JobPersistenceException
  {
    try
    {
  ...
  {
    return executeWithoutLock ( // no locks necessary
                                // for read...
                                conn -> getTriggersForJob (conn, jobKey));
  }

  protected List <OperableTrigger> getTriggersForJob (final Connection conn,
                                                      final JobKey key) throws JobPersistenceException
  {
   ...
   */
  public void pauseTrigger (final Connection conn, final TriggerKey triggerKey) throws JobPersistenceException
  {

    try
    {
      final String oldState = getDelegate ().selectTriggerState (conn, triggerKey);
      ...
      if (oldState.equals (STATE_WAITING) || oldState.equals (STATE_ACQUIRED))
      {
        ...
        if (oldState.equals (STATE_BLOCKED))
        {
  ...
  public void pauseJob (final JobKey jobKey) throws JobPersistenceException
  {
    executeInLock (LOCK_TRIGGER_ACCESS, new VoidTransactionCallback ()
    {
      ...
      public void executeVoid (final Connection conn) throws JobPersistenceException
      {
        final List <OperableTrigger> triggers = getTriggersForJob (conn, jobKey);
  ...
  public Set <String> pauseJobs (final GroupMatcher <JobKey> matcher) throws JobPersistenceException
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> {
      ...
      final Set <JobKey> jobNames = getJobNames (conn, matcher);
  ...
  /**
   * Determines if a Trigger for the given job should be blocked. State can only
   * transition to STATE_PAUSED_BLOCKED/BLOCKED from PAUSED/STATE_WAITING
   * respectively.
   *
   * @return STATE_PAUSED_BLOCKED, BLOCKED, or the currentState.
   */
  protected String checkBlockedState (final Connection conn,
                                      final JobKey jobKey,
                                      final String currentState) throws JobPersistenceException
  {
    ...
    try
    {
      final List <FiredTriggerRecord> lst = getDelegate ().selectFiredTriggerRecordsByJob (conn,
                                                                                           jobKey.getName (),
                                                                                           jobKey.getGroup ());

      if (lst.size () > 0)
      {
        final FiredTriggerRecord rec = lst.get (0);
        if (rec.isJobDisallowsConcurrentExecution ())
        { // OLD_TODO: worry about failed/recovering/volatile job states?
   ...
   */
  public void resumeTrigger (final Connection conn, final TriggerKey key) throws JobPersistenceException
  {
    try
    {

      final TriggerStatus status = getDelegate ().selectTriggerStatus (conn, key);
      ...
      final String newState = checkBlockedState (conn, status.getJobKey (), STATE_WAITING);
  ...
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> {
      final Set <JobKey> jobKeys = getJobNames (conn, matcher);
      final Set <String> groupNames = new HashSet<> ();
   ...
   * </p>
   *
   * @see #resumeTriggerGroup(java.sql.Connection,
   *      org.quartz.impl.matchers.GroupMatcher)
   */

  public Set <String> pauseTriggers (final GroupMatcher <TriggerKey> matcher) throws JobPersistenceException
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> pauseTriggerGroup (conn, matcher));
  }
   ...
   *      org.quartz.impl.matchers.GroupMatcher)
   */
  public Set <String> pauseTriggerGroup (final Connection conn,
                                         final GroupMatcher <TriggerKey> matcher) throws JobPersistenceException
  {
    ...
    try
    {
      ...
      getDelegate ().updateTriggerGroupStateFromOtherStates (conn,
                                                             matcher,
                                                             STATE_PAUSED,
                                                             STATE_ACQUIRED,
                                                             STATE_WAITING,
      ...
      final List <String> groups = getDelegate ().selectTriggerGroups (conn, matcher);

      // make sure to account for an exact group match for a group that doesn't
      // yet exist
  ...
  {
    return executeWithoutLock ( // no locks necessary for read...
                                conn -> getPausedTriggerGroups (conn));
  }
   ...
   */
  public Set <String> getPausedTriggerGroups (final Connection conn) throws JobPersistenceException
  {
  ...
  {
    return executeInLock (LOCK_TRIGGER_ACCESS, conn -> resumeTriggerGroup (conn, matcher));
   ...
   * @see #pauseTriggers(org.quartz.impl.matchers.GroupMatcher)
   */
  public Set <String> resumeTriggerGroup (final Connection conn,
                                          final GroupMatcher <TriggerKey> matcher) throws JobPersistenceException
  {
    ...
    try
    {
      ...
      final Set <TriggerKey> keys = getDelegate ().selectTriggersInGroup (conn, matcher);
      ...
      /*
       * int res = getDelegate().updateTriggerGroupStateFromOtherState(conn,
       * groupName, STATE_WAITING, PAUSED); if(res > 0) { long misfireTime =
       * System.currentTimeMillis(); if(getMisfireThreshold() > 0) misfireTime
       * -= getMisfireThreshold(); Key[] misfires =
       * getDelegate().selectMisfiredTriggersInGroupInState(conn, groupName,
       * STATE_WAITING, misfireTime); List blockedTriggers =
       * findTriggersToBeBlocked(conn, groupName); Iterator itr =
       * blockedTriggers.iterator(); while(itr.hasNext()) { Key key =
       * (Key)itr.next(); getDelegate().updateTriggerState(conn, key.getName(),
       * key.getGroup(), BLOCKED); } for(int i=0; i < misfires.length; i++) {
       * String newState = STATE_WAITING;
       * if(blockedTriggers.contains(misfires[i])) newState = BLOCKED;
       * updateMisfiredTrigger(conn, misfires[i].getName(),
   ...
   *
   * @see #resumeAll()
   * @see #pauseTriggerGroup(java.sql.Connection,
   *      org.quartz.impl.matchers.GroupMatcher)
   */
   ...
   *      org.quartz.impl.matchers.GroupMatcher)
   */
  public void pauseAll (final Connection conn) throws JobPersistenceException
  {

    final List <String> names = getTriggerGroupNames (conn);
   ...
   */
  public void resumeAll (final Connection conn) throws JobPersistenceException
  {
  ...
  /**
   * <p>
   * Get a handle to the next N triggers to be fired, and mark them as
   * 'reserved' by the calling scheduler.
   * </p>
   *
                                                     ...
                                                     final long timeWindow) throws JobPersistenceException
  {
    ...
    return executeInNonManagedTXLock (lockName,
                                      ...
                                      (conn, result) -> {
                                        try
                                        {
                                          final List <FiredTriggerRecord> acquired = getDelegate ().selectInstancesFiredTriggerRecords (conn,
                                                                                                                                        getInstanceId ());
                                          final Set <String> fireInstanceIds = new HashSet<> ();
                                          for (final FiredTriggerRecord ft : acquired)
                                          {
                                            fireInstanceIds.add (ft.getFireInstanceId ());
                                          }
                                          for (final OperableTrigger tr : result)
                                          {
  ...
  // FUTURE_TODO: this really ought to return something like a
  // FiredTriggerBundle,
  // so that the fireInstanceId doesn't have to be on the trigger...
  protected List <OperableTrigger> acquireNextTrigger (final Connection conn,
                                                       ...
                                                       final long timeWindow) throws JobPersistenceException
  {
    ...
    do
    {
      ...
      try
      {
        final List <TriggerKey> keys = getDelegate ().selectTriggerToAcquire (conn,
        ...
        for (final TriggerKey triggerKey : keys)
        {
          ...
          final OperableTrigger nextTrigger = retrieveTrigger (conn, triggerKey);
          if (nextTrigger == null)
          {
          ...
          // put it back into the timeTriggers set and continue to search for
          // next trigger.
          ...
          final int rowsUpdated = getDelegate ().updateTriggerStateFromOtherState (conn,
                                                                                   triggerKey,
                                                                                   STATE_ACQUIRED,
                                                                                   STATE_WAITING);
          if (rowsUpdated <= 0)
  ...
  protected void releaseAcquiredTrigger (final Connection conn,
                                         final OperableTrigger trigger) throws JobPersistenceException
  {
    try
    {
   ...
   * </p>
   *
   * @return null if the trigger or its job or calendar no longer exist, or if
   *         the trigger was not successfully put into the 'executing' state.
   */
  public List <TriggerFiredResult> triggersFired (final List <OperableTrigger> triggers) throws JobPersistenceException
  {
    return executeInNonManagedTXLock (LOCK_TRIGGER_ACCESS, conn -> {
      ...
      for (final OperableTrigger trigger : triggers)
      {
        try
        {
          final TriggerFiredBundle bundle = triggerFired (conn, trigger);
      ...
      try
      {
        final List <FiredTriggerRecord> acquired = getDelegate ().selectInstancesFiredTriggerRecords (conn,
                                                                                                      getInstanceId ());
        final Set <String> executingTriggers = new HashSet<> ();
        for (final FiredTriggerRecord ft : acquired)
        {
        ...
        for (final TriggerFiredResult tr : result)
        {
  ...
  protected TriggerFiredBundle triggerFired (final Connection conn,
                                             final OperableTrigger trigger) throws JobPersistenceException
  {
    JobDetail job;
    Calendar cal = null;
    ...
    { // if trigger was deleted, state will be STATE_DELETED
      final String state = getDelegate ().selectTriggerState (conn, trigger.getKey ());
    ...
    return new TriggerFiredBundle (job,
                                   trigger,
                                   cal,
  ...
  protected void triggeredJobComplete (final Connection conn,
    ...
    try
    {
      if (triggerInstCode == CompletedExecutionInstruction.DELETE_TRIGGER)
      {
        if (trigger.getNextFireTime () == null)
        {
          ...
          final TriggerStatus stat = getDelegate ().selectTriggerStatus (conn, trigger.getKey ());
  ...
  public void setLockHandler (final Semaphore lockHandler)
  {
  ...
  protected RecoverMisfiredJobsResult doRecoverMisfires () throws JobPersistenceException
  {
    ...
    final Connection conn = getNonManagedTXConnection ();
    try
    {
      ...
      final int misfireCount = (getDoubleCheckLockMisfireHandler ()) ? getDelegate ().countMisfiredTriggersInState (conn,
                                                                                                                    STATE_WAITING,
                                                                                                                    getMisfireTime ())
                                                                     : Integer.MAX_VALUE;
  ...
  protected void signalSchedulingChangeOnTxCompletion (final long candidateNewNextFireTime)
  {
    final Long sigTime = sigChangeForTxCompletion.get ();
  ...
  {
    final Long t = sigChangeForTxCompletion.get ();
    sigChangeForTxCompletion.set (null);
    return t;
  }

  protected void signalSchedulingChangeImmediately (final long candidateNewNextFireTime)
  {
  ...
  protected boolean doCheckin () throws JobPersistenceException
  {
    ...
    try
    {
      ...
      if (firstCheckIn || (failedRecords.size () > 0))
      {
        ...
        // The first time through, we also need to make sure we update/create
        // our state record
   ...
   */
  protected List <SchedulerStateRecord> findFailedInstances (final Connection conn) throws JobPersistenceException
  {
    try
    {
      final List <SchedulerStateRecord> failedInstances = new LinkedList<> ();
      boolean foundThisScheduler = false;
      final long timeNow = System.currentTimeMillis ();

      final List <SchedulerStateRecord> states = getDelegate ().selectSchedulerStateRecords (conn, null);

      for (final SchedulerStateRecord rec : states)
      {
      ...
      {
        // FUTURE_TODO: revisit when handle self-failed-out impl'ed (see
        // FUTURE_TODO in clusterCheckIn() below)
        getLog ().warn ("This scheduler instance (" +
                        getInstanceId () +
                        ") is still " +
   ...
   *
   * @param schedulerStateRecords
   *        List of all current <code>SchedulerStateRecords</code>
   */
  private List <SchedulerStateRecord> findOrphanedFailedInstances (final Connection conn,
                                                                   final List <SchedulerStateRecord> schedulerStateRecords) throws SQLException,
                                                                                                                            NoSuchDelegateException
  {
    final List <SchedulerStateRecord> orphanedInstances = new ArrayList<> ();

    final Set <String> allFiredTriggerInstanceNames = getDelegate ().selectFiredTriggerInstanceNames (conn);
    if (!allFiredTriggerInstanceNames.isEmpty ())
    {
      for (final SchedulerStateRecord rec : schedulerStateRecords)
      {

        allFiredTriggerInstanceNames.remove (rec.getSchedulerInstanceId ());
      }

      for (final String inst : allFiredTriggerInstanceNames)
      {

        final SchedulerStateRecord orphanedInstance = new SchedulerStateRecord ();
  ...
  protected long calcFailedIfAfter (final SchedulerStateRecord rec)
  {
  ...
  protected List <SchedulerStateRecord> clusterCheckIn (final Connection conn) throws JobPersistenceException
  {

    final List <SchedulerStateRecord> failedInstances = findFailedInstances (conn);
  ...
  protected void clusterRecover (final Connection conn,
                                 final List <SchedulerStateRecord> failedInstances) throws JobPersistenceException
  {
    ...
    if (failedInstances.size () > 0)
    {
      ...
      logWarnIfNonZero (failedInstances.size (),
                        "ClusterManager: detected " + failedInstances.size () + " failed or restarted instances.");
      try
      {
        for (final SchedulerStateRecord rec : failedInstances)
        {
          getLog ().info ("ClusterManager: Scanning for instance \"" +
                          rec.getSchedulerInstanceId () +
                          "\"'s failed in-progress jobs.");

          final List <FiredTriggerRecord> firedTriggerRecs = getDelegate ().selectInstancesFiredTriggerRecords (conn,
                                                                                                                rec.getSchedulerInstanceId ());
          ...
          for (final FiredTriggerRecord ftRec : firedTriggerRecs)
          {

            final TriggerKey tKey = ftRec.getTriggerKey ();
            final JobKey jKey = ftRec.getJobKey ();
            ...
            if (ftRec.getFireInstanceState ().equals (STATE_ACQUIRED))
            {
              ...
              if (ftRec.isJobRequestsRecovery ())
              {
                ...
                if (jobExists (conn, jKey))
                {
                  ...
                  final SimpleTriggerImpl rcvryTrig = new SimpleTriggerImpl ("recover_" +
                                                                             rec.getSchedulerInstanceId () +
                                                                             "_" +
                  ...
                  rcvryTrig.setMisfireInstruction (Trigger.MISFIRE_INSTRUCTION_IGNORE_MISFIRE_POLICY);
                  rcvryTrig.setPriority (ftRec.getPriority ());
                  final JobDataMap jd = getDelegate ().selectTriggerJobDataMap (conn,
                                                                                tKey.getName (),
                                                                                tKey.getGroup ());
                  jd.put (Scheduler.FAILED_JOB_ORIGINAL_TRIGGER_NAME, tKey.getName ());
                  jd.put (Scheduler.FAILED_JOB_ORIGINAL_TRIGGER_GROUP, tKey.getGroup ());
                  jd.put (Scheduler.FAILED_JOB_ORIGINAL_TRIGGER_FIRETIME_IN_MILLISECONDS,
                          String.valueOf (ftRec.getFireTimestamp ()));
                  jd.put (Scheduler.FAILED_JOB_ORIGINAL_TRIGGER_SCHEDULED_FIRETIME_IN_MILLISECONDS,
                          String.valueOf (ftRec.getScheduleTimestamp ()));
                ...
                {
                  getLog ().warn ("ClusterManager: failed job '" +
                                  jKey +
          ...
          // Check if any of the fired triggers we just deleted were the last
          // fired trigger
          ...
          for (final TriggerKey triggerKey : triggerKeys)
          {
            ...
            {
              final List <FiredTriggerRecord> firedTriggers = getDelegate ().selectFiredTriggerRecords (conn,
                                                                                                        triggerKey.getName (),
                                                                                                        triggerKey.getGroup ());
  ...
  protected void logWarnIfNonZero (final int val, final String warning)
  {
    if (val > 0)
    {
      getLog ().info (warning);
  ...
  /**
   * <p>
   * Cleanup the given database connection. This means restoring any modified
   * auto commit or transaction isolation connection attributes, and then
   * closing the underlying connection.
   * </p>
   * <p>
   * This is separate from closeConnection() because the Spring integration
   * relies on being able to overload closeConnection() and expects the same
   * connection back that it originally returned from the datasource.
   * </p>
   *
   * @see #closeConnection(Connection)
   */
  protected void cleanupConnection (final Connection conn)
  {
    if (conn != null)
    {
      if (conn instanceof Proxy)
      {
        final Proxy connProxy = (Proxy) conn;

        final InvocationHandler invocationHandler = Proxy.getInvocationHandler (connProxy);
        if (invocationHandler instanceof AttributeRestoringConnectionInvocationHandler)
        {
          final AttributeRestoringConnectionInvocationHandler connHandler = (AttributeRestoringConnectionInvocationHandler) invocationHandler;
   ...
   * <p>
   * Ignores a <code>null Connection</code>. Any exception thrown trying to
   * close the <code>Connection</code> is logged and ignored.
   * </p>
   *
   * @param conn
   *        The <code>Connection</code> to close (Optional).
   */
  protected void closeConnection (final Connection conn)
  {
    if (conn != null)
    {
      ...
      {
        getLog ().error ("Unexpected exception closing Connection." +
   ...
   * the exception lest it mask the exception that caused the caller to need to
   ...
   */
  protected void commitConnection (final Connection conn) throws JobPersistenceException
  {
  ...
  /**
   * Implement this interface to provide the code to execute within the a
   * transaction template. If no return value is required, execute should just
   * return null.
   *
  ...
  /**
   * Implement this interface to provide the code to execute within the a
   * transaction template that has no return value.
   *
   * @see JobStoreSupport#executeInNonManagedTXLock(String, TransactionCallback)
   */
  protected abstract class VoidTransactionCallback implements TransactionCallback <Void>
  {
    public final Void execute (final Connection conn) throws JobPersistenceException
    {
      executeVoid (conn);
      return null;
    }
  ...
  /**
   * Execute the given callback in a transaction. Depending on the JobStore, the
   * surrounding transaction may be assumed to be already present (managed).
   * <p>
   ...
   */
  public <T> T executeWithoutLock (final TransactionCallback <T> txCallback) throws JobPersistenceException
  {
    return executeInLock (null, txCallback);
  }

  /**
   * Execute the given callback having acquired the given lock. Depending on the
   * JobStore, the surrounding transaction may be assumed to be already present
   * (managed).
   *
   * @param lockName
   *        The name of the lock to acquire, for example "TRIGGER_ACCESS". If
   *        null, then no lock is acquired, but the lockCallback is still
   *        executed in a transaction.
   */
  protected abstract <T> T executeInLock (String lockName,
                                          TransactionCallback <T> txCallback) throws JobPersistenceException;

  protected <T> T retryExecuteInNonManagedTXLock (final String lockName, final TransactionCallback <T> txCallback)
  {
    for (int retry = 1; !shutdown; retry++)
    {
      ...
      try
      {
        Thread.sleep (getDbRetryInterval ()); // retry every N seconds (the db
  ...
  /**
   * Execute the given callback having optionally acquired the given lock. This
   * uses the non-managed transaction connection.
   *
   * @param lockName
   *        The name of the lock to acquire, for example "TRIGGER_ACCESS". If
   *        null, then no lock is acquired, but the lockCallback is still
   *        executed in a non-managed transaction.
   */
  protected <T> T executeInNonManagedTXLock (final String lockName,
                                             final TransactionCallback <T> txCallback,
                                             final TransactionValidator <T> txValidator) throws JobPersistenceException
  {
    boolean transOwner = false;
    ...
    try
    {
      ...
      catch (final JobPersistenceException e)
      {
        rollbackConnection (conn);
        if (txValidator == null ||
            !retryExecuteInNonManagedTXLock (lockName, conn1 -> txValidator.validate (conn1, result)))
        {
          throw e;
        }
      }

      final Long sigTime = clearAndGetSignalSchedulingChangeOnTxCompletion ();
  ...
  class ClusterManager extends Thread
  {
    ...
    public void initialize ()
    {
      ...
      final ThreadExecutor executor = getThreadExecutor ();
    ...
    public void run ()
    {
      while (!shutdown)
      {
        ...
        if (!shutdown)
        {
          ...
          final long transpiredTime = (System.currentTimeMillis () - lastCheckin);
  ...
  class MisfireHandler extends Thread
  {
    ...
    private RecoverMisfiredJobsResult manage ()
    {
      try
      {
        ...
        final RecoverMisfiredJobsResult res = doRecoverMisfires ();
        numFails = 0;
        return res;
      }
      catch (final Exception e)
    ...
    public void run ()
    {
      ...
      while (!shutdown)
      {
        ...
        final RecoverMisfiredJobsResult recoverMisfiredJobsResult = manage ();
        ...
        if (!shutdown)
        {
          long timeToSleep = 50l; // At least a short pause to help balance
                                  // threads
