...
public class UpdateChecker extends TimerTask
{
  ...
  private void doCheck () throws IOException
  {
    ...
    final URL updateUrl = buildUpdateCheckUrl ();
    final Properties updateProps = getUpdateProperties (updateUrl);
    final String currentVersion = getQuartzVersion ();
    ...
    if (notBlank (propVal))
    {
      ...
      final String [] newVersions = propVal.split (",");
      for (int i = 0; i < newVersions.length; i++)
      {
        final String newVersion = newVersions[i].trim ();
        if (i > 0)
        {
          sb.append (", ");
        }
  ...
  private Properties getUpdateProperties (final URL updateUrl) throws IOException
  {
  ...
  {
    final String url = System.getProperty ("quartz.update-check.url", UPDATE_CHECK_URL);
    final String connector = url.indexOf ('?') > 0 ? "&" : "?";
  ...
  private String urlEncode (final String param) throws UnsupportedEncodingException
  {
