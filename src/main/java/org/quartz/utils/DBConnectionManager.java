...
public class DBConnectionManager
{
  ...
  public void addConnectionProvider (final String dataSourceName, final ConnectionProvider provider)
  {
   ...
   *            if an error occurs, or there is no DataSource with the given
   *            name.
   */
  public Connection getConnection (final String dsName) throws SQLException
  {
    final ConnectionProvider provider = providers.get (dsName);
    if (provider == null)
    {
  ...
  /**
   * Shuts down database connections from the DataSource with the given name, if
   * applicable for the underlying provider.
   *
   * @exception SQLException
   *            if an error occurs, or there is no DataSource with the given
   *            name.
   */
  public void shutdown (final String dsName) throws SQLException
  {
  ...
  ConnectionProvider getConnectionProvider (final String key)
  {
