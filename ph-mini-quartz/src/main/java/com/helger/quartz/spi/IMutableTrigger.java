...
public interface IMutableTrigger extends ITrigger
{
  ...
  /**
   * The priority of a <code>Trigger</code> acts as a tie breaker such that if two
   * <code>Trigger</code>s have the same scheduled fire time, then Quartz will do its best to give
   * the one with the higher priority first access to a worker thread.<br>
   ...
   * properties of the trigger. However the first actual first time will not be before this
   * date.<br>
   * Setting a value in the past may cause a new trigger to compute a first fire time that is in the
   * past, which may cause an immediate misfire of the trigger.
   *
   * @param startTime
   *        start time
   */
   ...
   */
  void setMisfireInstruction (@NonNull EMisfireInstruction eMisfireInstruction);
