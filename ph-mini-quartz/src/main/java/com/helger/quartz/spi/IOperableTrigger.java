...
public interface IOperableTrigger extends IMutableTrigger
{
   ...
   * Called when the <code>IScheduler</code> has decided to 'fire' the trigger (execute the
   ...
   * The implementation should update the <code>Trigger</code>'s state based on the given new
  ...
  /**
   * This method should not be used by the Quartz client.<br>
   * Usable by <code>{@link com.helger.quartz.spi.IJobStore}</code> implementations, in order to
