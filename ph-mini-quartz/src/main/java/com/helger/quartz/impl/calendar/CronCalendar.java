...
/**
 * This implementation of the Calendar excludes the set of times expressed by a given
 * {@link com.helger.quartz.CronExpression CronExpression}. For example, you could use this calendar
 * to exclude all but business hours (8AM - 5PM) every day using the expression &quot;* * 0-7,18-23
 * ? * *&quot;.
 * <P>
 * It is important to remember that the cron expression here describes a set of times to be
 * <I>excluded</I> from firing. Whereas the cron expression in {@link com.helger.quartz.ICronTrigger
 * CronTrigger} describes a set of times that can be <I>included</I> for firing. Thus, if a
 * <CODE>CronTrigger</CODE> has a given cron expression and is associated with a
 * <CODE>CronCalendar</CODE> with the <I>same</I> expression, the calendar will exclude all the
 * times the trigger includes, and they will cancel each other out.
 *
 * @author Aaron Craven
 */
public class CronCalendar extends AbstractCalendar <CronCalendar>
{
  ...
  /**
   * Create a <CODE>CronCalendar</CODE> with the given cron exprssion, <CODE>baseCalendar</CODE>,
   ...
   *        a String representation of the desired cron expression
   * @param timeZone
   *        Specifies for which time zone the <code>expression</code> should be interpreted, i.e.
   *        the expression 0 0 10 * * ?, is resolved to 10:00 am in this time zone. If
   ...
   * <p>
   * Overrides <code>{@link AbstractCalendar#getTimeZone()}</code> to defer to its
   ...
   * will be resolved. If <code>timeZone</code> is <code>null</code> then
   * <code>TimeZone.getDefault()</code> will be used.
   * <p>
   * Overrides <code>{@link AbstractCalendar#setTimeZone(TimeZone)}</code> to defer to its
   * <code>CronExpression</code>.
   * </p>
   */
  @Override
