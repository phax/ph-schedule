...
public interface ISimpleTrigger extends ITrigger
{
   ...
   * situation, the <code>{@link ISimpleTrigger}</code> wants to be fired now by
   * <code>Scheduler</code>.
   * </p>
   * <p>
   ...
   * <p>
   * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire
   * situation, the <code>{@link ISimpleTrigger}</code> wants to be re-scheduled
   * to 'now' (even if the associated <code>{@link ICalendar}</code> excludes
   ...
   * <code>{@link ICalendar}</code>, and with the repeat count set to what it
   ...
   * situation, the <code>{@link ISimpleTrigger}</code> wants to be re-scheduled
   * to the next scheduled time after 'now' - taking into account any associated
   * <code>{@link ICalendar}</code>, and with the repeat count left unchanged.
   * </p>
   * <p>
   * <i>NOTE/WARNING:</i> This instruction could cause the <code>Trigger</code>
