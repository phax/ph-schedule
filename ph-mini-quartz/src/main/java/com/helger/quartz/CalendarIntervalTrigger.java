 ...
 * <code>{@link com.helger.quartz.JobDetail}</code> based upon repeating calendar time
 * intervals.
 * <p>
 ...
 * trigger's definition. This trigger can achieve schedules that are not
 * possible with {@link SimpleTrigger} (e.g because months are not a fixed
 * number of seconds) or {@link CronTrigger} (e.g. because "every 5 months" is
 * not an even divisor of 12).
 * </p>
 * <p>
...
public interface CalendarIntervalTrigger extends Trigger
{
   ...
   * <p>
   * Instructs the <code>{@link Scheduler}</code> that upon a mis-fire
   * situation, the <code>{@link CalendarIntervalTrigger}</code> wants to have
