...
/**
 * An exception that can be thrown by a <code>{@link com.helger.quartz.Job}</code> to
 * indicate to the Quartz <code>{@link Scheduler}</code> that an error occurred
 * while executing, and whether or not the <code>Job</code> requests to be
 * re-fired immediately (using the same
 * <code>{@link JobExecutionContext}</code>, or whether it wants to be
 * unscheduled.
 * <p>
