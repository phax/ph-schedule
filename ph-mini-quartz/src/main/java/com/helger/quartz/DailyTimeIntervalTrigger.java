...
/**
 * A <code>{@link Trigger}</code> that is used to fire a
 * <code>{@link com.helger.quartz.JobDetail}</code> based upon daily repeating time
 * intervals.
 * <p>
...
public interface DailyTimeIntervalTrigger extends Trigger
{
   ...
   * <p>
   * Instructs the <code>{@link Scheduler}</code> that upon a mis-fire
   * situation, the <code>{@link DailyTimeIntervalTrigger}</code> wants to have
