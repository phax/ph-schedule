 ...
 * <p>
 * <code>Triggers</code>s have a {@link TriggerKey} associated with them, which should uniquely
...
public interface ITrigger extends Comparable <ITrigger>
{
   ...
   * </p>
   * <p>
   * <code>RE_EXECUTE_JOB</code> Instructs the <code>{@link IScheduler}</code> that the
   * <code>{@link ITrigger}</code> wants the <code>{@link com.helger.quartz.IJobDetail}</code> to
   * re-execute immediately. If not in a 'RECOVERING' or 'FAILED_OVER' situation, the execution
   * context will be re-used (giving the <code>Job</code> the ability to 'see' anything placed in
   * the context by its last execution).
   * </p>
   * <p>
   * <code>SET_TRIGGER_COMPLETE</code> Instructs the <code>{@link IScheduler}</code> that the
   * <code>{@link ITrigger}</code> should be put in the <code>COMPLETE</code> state.
   * </p>
   * <p>
   * <code>DELETE_TRIGGER</code> Instructs the <code>{@link IScheduler}</code> that the
   * <code>{@link ITrigger}</code> wants itself deleted.
   * </p>
   * <p>
   * <code>SET_ALL_JOB_TRIGGERS_COMPLETE</code> Instructs the <code>{@link IScheduler}</code> that
   * all <code>Trigger</code>s referencing the same
   * <code>{@link com.helger.quartz.IJobDetail}</code> as this one should be put in the
   * <code>COMPLETE</code> state.
   * </p>
   * <p>
   * <code>SET_TRIGGER_ERROR</code> Instructs the <code>{@link IScheduler}</code> that all
   * <code>Trigger</code>s referencing the same <code>{@link com.helger.quartz.IJobDetail}</code> as
   * this one should be put in the <code>ERROR</code> state.
   * </p>
   * <p>
   * <code>SET_ALL_JOB_TRIGGERS_ERROR</code> Instructs the <code>{@link IScheduler}</code> that the
   * <code>Trigger</code> should be put in the <code>ERROR</code> state.
   * </p>
   */
  ...
  {
    /**
     * Instructs the <code>{@link IScheduler}</code> that the <code>Trigger</code> will never be
     * evaluated for a misfire situation, and that the scheduler will simply try to fire it as soon
     * as it can, and then update the Trigger as if it had fired at the proper time.<br>
     * NOTE: if a trigger uses this instruction, and it has missed several of its scheduled firings,
     * then several rapid firings may occur as the trigger attempt to catch back up to where it
     * would have been. For example, a SimpleTrigger that fires every 15 seconds which has misfired
     * for 5 minutes will fire 20 times once it gets the chance to fire.
     */
    MISFIRE_INSTRUCTION_IGNORE_MISFIRE_POLICY, // -1

    /**
     * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire situation, the
     * <code>updateAfterMisfire()</code> method will be called on the <code>Trigger</code> to
     * determine the mis-fire instruction, which logic will be trigger-implementation-dependent.<br>
     * In order to see if this instruction fits your needs, you should look at the documentation for
     * the <code>getSmartMisfirePolicy()</code> method on the particular <code>Trigger</code>
    ...
    /**
     * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire situation, the
     * <code>Trigger</code> wants to be fired now by <code>Scheduler</code>.
     */
    MISFIRE_INSTRUCTION_FIRE_ONCE_NOW, // 1
    /**
     * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire situation, the
     * <code>Trigger</code> wants to have it's next-fire-time updated to the next time in the
     * schedule after the current time (taking into account any associated <code>Calendar</code>,
     * but it does not want to be fired now.<br>
    ...
    /**
     * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire situation, the
     * <code>{@link ISimpleTrigger}</code> wants to be re-scheduled to 'now' (even if the associated
     * <code>{@link ICalendar}</code> excludes 'now') with the repeat count left as-is. This does
     * obey the <code>Trigger</code> end-time however, so if 'now' is after the end-time the
     * <code>Trigger</code> will not fire again.<br>
     * <i>NOTE:</i> Use of this instruction causes the trigger to 'forget' the start-time and
     * repeat-count that it was originally setup with (this is only an issue if you for some reason
     * wanted to be able to tell what the original values were at some later time).<br>
     ...
     * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire situation, the
     * <code>{@link ISimpleTrigger}</code> wants to be re-scheduled to 'now' (even if the associated
     * <code>{@link ICalendar}</code> excludes 'now') with the repeat count set to what it would be,
     * if it had not missed any firings. This does obey the <code>Trigger</code> end-time however,
     * so if 'now' is after the end-time the <code>Trigger</code> will not fire again.<br>
     * <i>NOTE:</i> Use of this instruction causes the trigger to 'forget' the start-time and
     ...
     * reason wanted to be able to tell what the original values were at some later time).<br>
     * <i>NOTE:</i> This instruction could cause the <code>Trigger</code> to go to the 'COMPLETE'
     * state after firing 'now', if all the repeat-fire-times where missed.<br>
     * Only applicable to ISimpleTrigger
     */
    ...
    /**
     * Instructs the <code>{@link IScheduler}</code> that upon a mis-fire situation, the
     * <code>{@link ISimpleTrigger}</code> wants to be re-scheduled to the next scheduled time after
     * 'now' - taking into account any associated <code>{@link ICalendar}</code>, and with the
     * repeat count set to what it would be, if it had not missed any firings.<br>
     * <i>NOTE/WARNING:</i> This instruction could cause the <code>Trigger</code> to go directly to
     * the 'COMPLETE' state if all fire-times where missed.<br>
     * Only applicable to ISimpleTrigger
     */
     ...
     * <code>{@link ISimpleTrigger}</code> wants to be re-scheduled to the next scheduled time after
     * 'now' - taking into account any associated <code>{@link ICalendar}</code>, and with the
     * repeat count left unchanged.<br>
     * <i>NOTE/WARNING:</i> This instruction could cause the <code>Trigger</code> to go directly to
     * the 'COMPLETE' state if the end-time of the trigger has arrived.<br>
     * Only applicable to ISimpleTrigger
     */
  ...
  /**
   * Get the name of the <code>{@link ICalendar}</code> associated with this Trigger.
   *
  ...
  /**
   * Get the <code>JobDataMap</code> that is associated with the <code>Trigger</code>.<br>
   * Changes made to this map during job execution are not re-persisted, and in fact typically
   ...
   * <code>Trigger</code>s have the same scheduled fire time, then the one with the higher priority
   * will get first access to a worker thread.<br>
  ...
  /**
   * Used by the <code>{@link IScheduler}</code> to determine whether or not it is possible for this
   * <code>Trigger</code> to fire again.
   * <p>
   * If the returned value is <code>false</code> then the <code>Scheduler</code> may remove the
   * <code>Trigger</code> from the <code>{@link com.helger.quartz.spi.IJobStore}</code>.
   * </p>
   */
   ...
   *
   * @return <code>true</code> if the key of this Trigger equals that of the given Trigger.
   */
