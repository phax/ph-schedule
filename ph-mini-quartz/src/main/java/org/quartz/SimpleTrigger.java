...
/**
 * A <code>{@link Trigger}</code> that is used to fire a <code>Job</code> at a
...
public interface SimpleTrigger extends Trigger
{
   ...
   * situation, the <code>{@link SimpleTrigger}</code> wants to be fired now by
   * <code>Scheduler</code>.
   * </p>
   * <p>
   ...
   * <code>{@link #MISFIRE_INSTRUCTION_RESCHEDULE_NOW_WITH_REMAINING_REPEAT_COUNT}
   * </code>.
   * </p>
   */
   ...
   * <p>
   * Instructs the <code>{@link Scheduler}</code> that upon a mis-fire
   * situation, the <code>{@link SimpleTrigger}</code> wants to be re-scheduled
   * to 'now' (even if the associated <code>{@link Calendar}</code> excludes
   * 'now') with the repeat count left as-is. This does obey the
   * <code>Trigger</code> end-time however, so if 'now' is after the end-time
   * the <code>Trigger</code> will not fire again.
   * </p>
   * <p>
   * <i>NOTE:</i> Use of this instruction causes the trigger to 'forget' the
   * start-time and repeat-count that it was originally setup with (this is only
   * an issue if you for some reason wanted to be able to tell what the original
   ...
   * situation, the <code>{@link SimpleTrigger}</code> wants to be re-scheduled
   * to 'now' (even if the associated <code>{@link Calendar}</code> excludes
   * 'now') with the repeat count set to what it would be, if it had not missed
   * any firings. This does obey the <code>Trigger</code> end-time however, so
   * if 'now' is after the end-time the <code>Trigger</code> will not fire
   * again.
   * </p>
   * <p>
   * <i>NOTE:</i> Use of this instruction causes the trigger to 'forget' the
   * start-time and repeat-count that it was originally setup with. Instead, the
   * repeat count on the trigger will be changed to whatever the remaining
   * repeat count is (this is only an issue if you for some reason wanted to be
   * able to tell what the original values were at some later time).
   * </p>
   * <p>
   * <i>NOTE:</i> This instruction could cause the <code>Trigger</code> to go to
   * the 'COMPLETE' state after firing 'now', if all the repeat-fire-times where
   * missed.
   * </p>
   */
   ...
   * <code>{@link Calendar}</code>, and with the repeat count set to what it
   ...
   * situation, the <code>{@link SimpleTrigger}</code> wants to be re-scheduled
   * to the next scheduled time after 'now' - taking into account any associated
   * <code>{@link Calendar}</code>, and with the repeat count left unchanged.
   * </p>
   * <p>
   * <i>NOTE/WARNING:</i> This instruction could cause the <code>Trigger</code>
   * to go directly to the 'COMPLETE' state if the end-time of the trigger has
   * arrived.
   * </p>
   */
